create table if not exists public.not_notes_documents (
  user_id uuid primary key references auth.users(id) on delete cascade,
  payload jsonb not null default '{"folders":[],"notes":[],"dark":false}'::jsonb,
  updated_at timestamptz not null default now()
);

alter table public.not_notes_documents enable row level security;
...
do $$
begin
  alter publication supabase_realtime add table public.not_notes_documents;
exception
