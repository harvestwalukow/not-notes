...
function NoteEditor({ note, updateNoteBody, deleteNote, onBack, mobileView, saveLabel }) {
  ...
  return (
    <main className={`editor-panel ${mobileView === 'editor' ? 'mobile-current' : ''}`}>
      ...
      <div ref={editorRef} className="note-canvas" contentEditable suppressContentEditableWarning onInput={() => { save(); rememberSelection() }} onMouseUp={rememberSelection} onKeyUp={rememberSelection} onKeyDown={handleKeyDown} onBlur={rememberSelection} onClick={handleChecklistClick} onPaste={handlePaste} aria-label={`Editing ${note.title}`}/>
